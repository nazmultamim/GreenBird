...
export default async function FollowingPage() {
      ...
      <div className="min-h-screen text-foreground">
        ...
        <div className="mx-auto flex max-w-[1420px] justify-center">
          ...
          <main className="min-h-screen w-full max-w-[680px] border-x emerald-divider bg-[#02120e]/70 p-6">
            <p className="text-sm text-emerald-100/60">Please sign in to see who you follow.</p>
          </main>
          <RightSidebar />
        </div>
      </div>
  ...
  const following = await User.find({ _id: { $in: currentUser.following || [] } }).lean();
      ...
      <div className="mx-auto flex max-w-[1420px] justify-center">
        ...
        <main className="min-h-screen w-full max-w-[680px] border-x emerald-divider bg-[#02120e]/70 pt-14 pb-10 shadow-[0_0_80px_rgba(0,0,0,0.22)] backdrop-blur-xl sm:pt-0">
          <div className="p-4">
            ...
            <p className="mt-2 text-sm text-emerald-100/60">Accounts you follow</p>

            <div className="mt-6 space-y-3">
              {following.length === 0 ? (
                <div className="text-emerald-100/60">Not following anyone yet.</div>
              ) : (
                following.map((u) => (
                  <div key={u._id} className="flex items-center gap-3 rounded-md p-3 hover:bg-white/2">
                    ...
                    <div>
                      <FollowButton username={u.username} initialFollowing={true} initialCount={u.followers?.length || 0} />
                    </div>
                  </div>
                ))
              )}
